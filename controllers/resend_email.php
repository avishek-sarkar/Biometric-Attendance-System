...
try {
    ...
    $check_stmt = $conn->prepare("SELECT * FROM temp_registrations WHERE email = ? AND is_verified = 0");
    ...
    $update_stmt = $conn->prepare("UPDATE temp_registrations SET verification_token = ? WHERE email = ?");
    ...
    <a href='http://localhost:81/Biometric-Attendance-System/views/mail_verification.php?token=$verification_token' 
